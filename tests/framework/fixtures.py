    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        for i in range(50):
            if i % 5 == 0:
                ...
                commits.append(f"Change {i+1}: This is a very long commit message that will definitely wrap to multiple lines when displayed in the narrow commits pane. It contains enough text to thoroughly test the multi-line display behavior of tigs terminal interface and should cause issues with cursor navigation if not handled properly.")
            elif i % 7 == 0:
                # Every 7th commit: Medium length that might wrap
                commits.append(f"Change {i+1}: Implement feature with moderately long description that may wrap depending on terminal width and column layout configuration")
            elif i % 3 == 0:
                # Every 3rd commit: Multi-line with actual newlines
                commits.append(f"Change {i+1}: Feature implementation\n\nThis commit adds the following important changes:\n- Feature A implementation\n- Feature B with error handling\n- Bug fixes and improvements\n- Documentation updates")
            else:
...
def extreme_repo() -> Generator[Path, None, None]:
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        extreme_commits = [
            ...
            "Commit 1: " + "x" * 500 + " This commit has an extremely long single line that should definitely cause wrapping issues and potentially break terminal display parsing in ways that reveal cursor navigation bugs",
            ...
            "Commit 4: " + "Very long line " * 50 + "\n\nSecond paragraph is also very long " + "repeated text " * 30,
            ...
            "Commit 8: " + "This is another extremely long commit message designed to wrap multiple lines and potentially cause issues with terminal display parsing when the text extends beyond normal boundaries " * 3,
            
            # Commit 9: Mixed content
            "Commit 9: Mixed\n\nBullet points:\n• Item 1 with very long text " + "x" * 100 + "\n• Item 2\n• Item 3 with more text " + "y" * 80,
