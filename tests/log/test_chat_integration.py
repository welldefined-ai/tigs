        ...
        with TUI(command, cwd=PYTHON_DIR, dimensions=(30, 140)) as tui:
            try:
                ...
                for i in range(4):  # Try moving through several commits
                    ...
                    from framework.tui import find_cursor_row, get_first_pane
                    try:
                ...
                first_move_indicator = initial_lines[0] if initial_lines else ""
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with TUI(command, cwd=PYTHON_DIR, dimensions=(30, 140)) as tui:
                try:
                    ...
                    non_empty_lines = [l for l in chat_lines if l.strip()]
