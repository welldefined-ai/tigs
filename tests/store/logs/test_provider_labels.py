...
class TestLogProviderLabels:
    ...
    def test_provider_name_visible(self, repo_with_provider):
        ...
        with TUI(
            ...
            if "Logs" in combined or "▶" in combined:
                assert "Claude" in combined, f"Logs pane present but provider label missing: {combined}"
    ...
    def test_wrapped_two_line_format(self, repo_with_provider):
        ...
        with TUI(
            ...
            for line in lines:
                ...
                if 'x' in line:
                    ...
                    if len(parts) >= 2:
                        ...
                        if logs_part and not logs_part.startswith('(') and logs_part != '':
                            logs_content.append(logs_part)

            # Fail if logs pane didn't appear
            if not logs_content:
                pytest.fail(f"Logs pane has no content. Full output:\n{chr(10).join(lines)}")
            ...
            assert "Claude" in first_line, f"Line 0 must contain provider 'Claude', got: '{first_line}'"
            # STRICT CHECK: Must have full MM-DD date format (not truncated)
            date_match = re.search(r'\d{2}-\d{2}', first_line)
            assert date_match, f"Line 0 must contain FULL date (MM-DD), got: '{first_line}'. Date was truncated - logs pane too narrow!"
            ...
            if len(logs_content) > 1:
                ...
                assert time_match, f"Line 1 must contain time (HH:MM), got: '{second_line}'"
                # Verify it's primarily just the time (not mixed with other content)
                time_only = re.match(r'^\s*\d{2}:\d{2}\s*$', second_line)
                assert time_only, f"Line 1 should be only time with optional whitespace, got: '{second_line}'"
            else:
                pytest.fail(f"Expected at least 2 lines for wrapped format, got {len(logs_content)}: {logs_content}")
