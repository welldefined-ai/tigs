...
class TestCLIErrors:
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            for cmd in commands:
                ...
                assert any(indicator in error_output.lower() for indicator in 
                          ["not a git repository", "not git repository", "git repo", "invalid repo"])

    def test_invalid_repo_path(self):
        """Test with a non-existent repository path."""
        result = run_tigs("/nonexistent/path", "list-chats")
        print(f"Invalid path result: {result.returncode}, {result.stdout}, {result.stderr}")
    ...
    def test_empty_repository(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            print(f"Empty repo add-chat: {result.returncode}, {result.stdout}, {result.stderr}")
            
            # Should fail gracefully
            assert result.returncode != 0
            error_output = result.stdout + result.stderr
            assert any(indicator in error_output.lower() for indicator in 
                      ["no commit", "invalid commit", "no head", "empty"])
    ...
    def test_invalid_commit_sha(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            error_output = result.stdout + result.stderr
            assert any(indicator in error_output.lower() for indicator in 
                      ["invalid commit", "not found", "bad commit"])
    ...
    def test_sync_without_remote(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            print(f"Push without remote: {result.returncode}, {result.stdout}, {result.stderr}")
            
            # Should fail gracefully
            assert result.returncode != 0
            error_output = result.stdout + result.stderr
            assert any(indicator in error_output.lower() for indicator in 
                      ["remote", "not found", "error", "does not exist"])
            
            # Test fetch without remote
            result = run_tigs(repo_path, "fetch-chats", "nonexistent")
            print(f"Fetch without remote: {result.returncode}, {result.stdout}, {result.stderr}")
    ...
    def test_malformed_yaml_content(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            print(f"Malformed YAML: {result.returncode}, {result.stdout}, {result.stderr}")
    ...
    def test_permission_errors(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            try:
                ...
                print(f"Permission error: {result.returncode}, {result.stdout}, {result.stderr}")
