...
class TestMessagesViewColors:
                ...
                if isinstance(line, list):
                    for part in line:
                        assert isinstance(part, tuple), (
                            f"Expected tuple, got {type(part)}: {part}"
                        )
                    ...
                    for text, color in line:
                        if text == "User":
                            assert color == COLOR_DEFAULT, (
                                "User role should be default color"
                            )
                            break
            ...
            if isinstance(line, list):
                ...
                if "Assistant" in text_combined and ":" in text_combined:
                    ...
                    for text, color in line:
                        if text == "Assistant":
                            found = True
                            assert color == COLOR_CYAN, (
                                f"Assistant role should be cyan, got {color}"
                            )
                            break
                    ...
                    for text, color in line:
                        if text == "System":
                            ...
                            assert color == COLOR_YELLOW, (
                                f"System role should be yellow, got {color}"
                            )
                            break
                ...
                for text, color in line:
                    ...
                    if " 09-10 " in text or " 14:3" in text:
                        assert color == COLOR_METADATA, (
                            f"Timestamp should be blue: '{text}'"
                        )
                ...
                if text_combined.startswith("    ") and text_combined.strip():
                    ...
                    for text, color in line:
                        if text.strip() and not text.isspace():
                            assert color == COLOR_DEFAULT, (
                                f"Content should be default: '{text}'"
                            )
        ...
        for line in lines:
            if isinstance(line, list) and line:
                ...
                if "[ ]" in text or "[x]" in text or ">" in text:
                    assert color == COLOR_DEFAULT, (
                        f"Selection indicator should be default: '{text}'"
                    )
    ...
    def test_wrapped_content_maintains_color(self):
        ...
        for line in lines:
            if isinstance(line, list):
                ...
                if text.startswith("    ") and (
                    "long message" in text.lower()
                    or "wrap" in text.lower()
                    or "displayed" in text.lower()
                ):
                    wrapped_count += 1
        ...
        assert wrapped_count > 0, (
            f"No wrapped lines found. Lines: {[(''.join(t for t, _ in line) if isinstance(line, list) else line) for line in lines]}"
        )
    ...
    def test_color_consistency_across_width_changes(self):
        ...
        lines_wide = self.view.get_display_lines(
            height=30, width=80, colors_enabled=True
        )
        lines_narrow = self.view.get_display_lines(
            height=30, width=30, colors_enabled=True
        )
