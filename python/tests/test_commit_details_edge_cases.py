            ...
            for line in lines:
                if isinstance(line, list):
                    ...
                    if "Bin" in text and " | " in text:
                        ...
                        assert line[0][1] == 7, (
                            f"Binary file should have blue filename: {text}"
                        )

            assert binary_count == 2, (
                f"Should find 2 binary files, found {binary_count}"
            )
...
{chr(10).join(file_lines)}
 15 files changed, 120 insertions(+)"""
        ...
        with patch("subprocess.run") as mock_run:
            ...
            lines = self.view.get_display_lines(
                height=height, width=80, colors_enabled=True
            )
            ...
            for line in lines:
                if isinstance(line, list):
                    ...
                    if " | " in text and "file" in text:
                        # Should be blue
                        assert line[0][1] == 7, (
                            "File at bottom should have blue filename"
                        )
...
AuthorDate: Mon Sep 10 14:30:00 2025 +0800
 ...
 1 file changed, 50 insertions(+)"""
        ...
        with patch("subprocess.run") as mock_run:
            ...
            lines = self.view.get_display_lines(
                height=20, width=35, colors_enabled=True
            )
            ...
            for line in lines:
                if isinstance(line, list):
                    text = "".join(t for t, _ in line)
                    if any(
                        x in text for x in ["extremely_long", "definitely_wrap", ".txt"]
                    ):
