...
class TestCommitsViewColors:
        ...
        for line in lines:
            if line:  # Skip empty lines
                assert isinstance(line, list), (
                    f"Expected list, got {type(line)}: {line}"
                )
                for part in line:
                    ...
                    if any(c.isdigit() for c in text) and ("-" in text or ":" in text):
                        if len(text.strip()) > 3:  # Not just selection marker
                            assert color == COLOR_METADATA, (
                                f"DateTime should be blue: '{text}'"
                            )
                    ...
                    for text, color in line:
                        if "Add new feature" in text or "Fix bug" in text:
                            assert color == COLOR_DEFAULT, (
                                f"Subject should be default: {text}"
                            )
                ...
                if text_parts.startswith("    "):
                    ...
                    for text, color in line:
                        if text.strip():  # Non-whitespace content
                            assert color == COLOR_DEFAULT, (
                                f"Wrapped content should be default: {text}"
                            )
                ...
                if "VeryLongAuthor" in text_parts:
                    ...
                    for text, color in line:
                        if "VeryLongAuthor" in text:
                            assert color == COLOR_AUTHOR, (
                                f"Long author should be cyan: {text}"
                            )
        ...
        for line in lines:
            if isinstance(line, list) and line:
                ...
                if first_text.strip() in ["[ ]", "[x]", ">", ">  ", "   "]:
                    assert first_color == COLOR_DEFAULT, (
                        f"Selection marker should be default: '{first_text}'"
                    )
                ...
                if "VISUAL" in text:
                    for part_text, part_color in last_line:
                        assert part_color == COLOR_DEFAULT, (
                            f"Visual mode should be default: {part_text}"
                        )
    ...
    def test_color_consistency_across_width_changes(self):
        ...
        lines_wide = self.view.get_display_lines(
            height=20, width=100, colors_enabled=True
        )
        lines_narrow = self.view.get_display_lines(
            height=20, width=40, colors_enabled=True
        )
