 ...
 2 files changed, 103 insertions(+), 2 deletions(-)"""
        ...
        with patch("subprocess.run") as mock_run:
            ...
            lines = self.view.get_display_lines(
                height=30, width=50, colors_enabled=True
            )
            ...
            for i, line in enumerate(lines):
                if isinstance(line, list):
                    ...
                    if "very_long_filename" in full_text:
                        ...
                        in_long_file
                        and "+" in full_text
                        and "another_file" not in full_text
                        and "files changed" not in full_text
                    ):
            ...
            assert len(long_file_lines) > 1, (
                "Long filename should wrap into multiple lines"
            )
            ...
            for text, color in first_line:
                if "very_long_filename" in text or "very_long" in text or " | " in text:
                    ...
                    if " | " in text and color == 7:
                        has_blue_filename = True
            assert has_blue_filename, (
                f"First line should contain blue filename. Line was: {first_line}"
            )

            # Check wrapped continuation lines if any
            if len(long_file_lines) > 1:
                for wrapped_line in long_file_lines[1:]:
                    assert isinstance(wrapped_line, list), (
                        f"Wrapped lines should also be multi-colored, got {wrapped_line}"
                    )
                    ...
                    for text, color in wrapped_line:
                        if "+" in text:
                            ...
                            for char in text:
                                if char == "+":
                                    assert color == 3, (
                                        f"Plus signs should be green (3), got {color}"
                                    )
...
abc123def456789abcdef0123456789abcdef0123456789 refs/tags/release-candidate"""
            ...
            self.view.load_commit_details(
                "abc123def456789abcdef0123456789abcdef0123456789"
            )
            # Very narrow width
            lines = self.view.get_display_lines(
                height=50, width=30, colors_enabled=True
            )
            ...
            for line in lines:
                if isinstance(line, tuple):
                    text, color = line
                    if text.startswith("commit") or (
                        "abc123def456789" in text and color == 3
                    ):
                        # Commit line may wrap, but all parts should be green
                        if color == 3:
                            has_green_commit = True
                    elif text.startswith("Author:") or (
                        "john" in text.lower() and color == 2
                    ):
...
AuthorDate: Mon Sep 10 14:30:00 2025 +0800
 ...
 3 files changed"""
        ...
        with patch("subprocess.run") as mock_run:
            ...
            lines = self.view.get_display_lines(
                height=20, width=80, colors_enabled=True
            )
            ...
            for line in lines:
                if isinstance(line, list):
                    ...
                    if "=>" in full_text:
                        ...
                        assert has_blue, (
                            f"Renamed file should have blue parts: {full_text}"
                        )
 ...
 3 files changed, 6 insertions(+), 3 deletions(-)"""
        ...
        with patch("subprocess.run") as mock_run:
            ...
            for line in lines:
                if isinstance(line, list):
                    full_text = "".join(text for text, _ in line)
                    if " | " in full_text and any(
                        c in full_text for c in ['"', "(", ")", "[", "]"]
                    ):
                        found_files.append(full_text)
                        # First part should be blue
                        assert line[0][1] == 7, (
                            f"Filename with special chars should be blue: {full_text}"
                        )

            assert len(found_files) == 3, (
                f"Should find all 3 files with special chars, found: {found_files}"
            )
...
AuthorDate: Mon Sep 10 14:30:00 2025 +0800
 ...
 1 file changed, 200 insertions(+)"""
        ...
        with patch("subprocess.run") as mock_run:
            ...
            self.view.load_commit_details("abc123")
            lines = self.view.get_display_lines(
                height=50, width=40, colors_enabled=True
            )
            ...
            for text, color in plus_lines:
                for char in text:
                    if char == "+":
                        ...
                        assert color == 3, (
                            "Plus signs should be green even in wrapped lines"
                        )

            assert total_plus_count == 200, (
                f"Should have all 200 + symbols colored, got {total_plus_count}"
            )
                    ...
                    if "mixed.txt" in full_text or "balanced.py" in full_text:
                        ...
                        for text, color in line:
                            ...
                            if ".txt" in text or ".py" in text:
                                assert color == 7 or color == 0, (
                                    "Filename should be blue or default"
                                )
                            elif "+" in text:
                                for char in text:
                                    if char == "+":
        ...
        with patch("subprocess.run") as mock_run:
            ...
            for line in lines:
                assert isinstance(line, str), (
                    f"With colors_enabled=False, should return strings, got {type(line)}"
                )
...
{file_lines}
 30 files changed, 465 insertions(+)"""
        ...
        with patch("subprocess.run") as mock_run:
            ...
            # Get first page
            lines1 = self.view.get_display_lines(
                height=10, width=80, colors_enabled=True
            )

            # Scroll down
            self.view.scroll_down(lines=5, viewport_height=10)
            lines2 = self.view.get_display_lines(
                height=10, width=80, colors_enabled=True
            )
                ...
                for line in lines:
                    if isinstance(line, list):
                        ...
                        if line[0][0].strip().startswith("file"):
                            assert line[0][1] == 7, (
                                "Filename should be blue after scrolling"
                            )
