...
AuthorDate: Mon Sep 10 14:30:00 2025 +0800
 ...
 1 file changed, 10 insertions(+)"""
        ...
        with patch("subprocess.run") as mock_run:
            ...
            lines = self.view.get_display_lines(
                height=30, width=40, colors_enabled=True
            )
            ...
            for i, line in enumerate(lines):
                if isinstance(line, tuple):
                    ...
                    if text.startswith("commit ") or (i > 0 and "abc123" in text):
                        ...
                        "Author" in text
                        or ("john" in text.lower() and color == 2)
            ...
            for text, color in commit_lines:
                assert color == 3, (
                    f"Commit line part '{text}' should be green (3), got {color}"
                )

            # All author line parts should be cyan
            for text, color in author_lines:
                assert color == 2, (
                    f"Author line part '{text}' should be cyan (2), got {color}"
                )
 ...
 3 files changed, 13 insertions(+), 5 deletions(-)"""
        ...
        with patch("subprocess.run") as mock_run:
            ...
            assert len(file_lines) >= 3, (
                f"Should have at least 3 file change lines, got {len(file_lines)}"
            )
            ...
            for parts in file_lines:
                ...
                assert " | " in filename_part, (
                    f"First part should contain filename and separator: {filename_part}"
                )
                assert filename_color == 7, (
                    f"Filename should be blue (7), got {filename_color}"
                )
                ...
                if "added.txt" in full_text:
                    ...
                    for text, color in parts[1:]:
                        if "+" in text:
                            assert color == 3, (
                                f"Additions (+) should be green (3), got {color}"
                            )
                    ...
                    for text, color in parts[1:]:
                        if "-" in text:
                            assert color == 6, (
                                f"Deletions (-) should be red (6), got {color}"
                            )
                    ...
                    assert has_green and has_red, (
                        "Modified file should have both green and red parts"
                    )
            ...
            for line in lines:
                if isinstance(line, tuple):
                    ...
                    if "insertions(+)" in text and "deletions(-)" not in text:
                        assert color == 3, (
                            f"Insertions-only summary should be green (3), got {color}"
                        )
                        break
