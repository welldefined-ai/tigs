...
class TestTUIStoreEndToEnd:
    ...
    def test_store_operation_error_handling_real_git(
        self, git_repo, sample_yaml_content
    ):
        ...
        app.commit_view.get_selected_shas.return_value = [
            "invalid-sha-that-does-not-exist"
        ]
    ...
    def test_store_with_existing_notes_overwrite(
        ...
        result = runner.invoke(
            main, ["--repo", str(git_repo), "add-chat", "-m", "Original content"]
        )
    ...
    def test_full_tui_workflow_simulation(
        self, multi_commit_repo, claude_logs, git_notes_helper
    ):
...
class TestTUIDynamicLayout:
    ...
    def test_commit_title_soft_wrap(self):
        ...
        with patch("subprocess.run") as mock_run:
            ...
            mock_run.return_value.stdout = (
                "abc123|" + ("A" * 100) + "|Author|1234567890"
            )
            ...
            assert re.search(date_pattern, first_line), (
                f"Expected MM-DD HH:MM pattern in: {first_line}"
            )
    ...
    def test_dynamic_width_calculation(self):
        ...
        with patch("subprocess.run") as mock_run:
            ...
            commit_w, msg_w, log_w = app.layout_manager.calculate_column_widths(
                120, [c["subject"] for c in app.commit_view.commits], 5
            )
            ...
            commit_w2, msg_w2, log_w2 = app.layout_manager.calculate_column_widths(
                80, [c["subject"] for c in app.commit_view.commits], 5
            )

            # Should still meet minimums
            assert (
                commit_w2 >= app.layout_manager.MIN_COMMIT_WIDTH
                or msg_w2 >= app.layout_manager.MIN_MESSAGE_WIDTH
            )
    ...
    def test_variable_message_heights(self):
        ...
        heights = app.message_view._calculate_message_heights(
            app.message_view.messages, 30
        )
        ...
        visible_count, start_idx, end_idx = (
            app.message_view._get_visible_messages_variable(15, heights)
        )
