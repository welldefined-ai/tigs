            ...
            for line in lines:
                if isinstance(line, list):
                    ...
                    if "=>" in text and " | 0" in text:
                        ...
                        assert line[0][1] == 7, (
                            f"Renamed file should have blue filename: {text}"
                        )
                    ...
                    if "=>" in text and " | 0" in text:
                        ...
                        assert False, (
                            f"Rename with 0 changes should be multi-colored, not black: {text}"
                        )

            assert len(rename_lines) == 3, (
                f"Should find 3 rename lines, found {len(rename_lines)}"
            )
