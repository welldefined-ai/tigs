...
class LayoutManager:
    ...
    def calculate_column_widths(
        ...
        if log_count == 0:
            ...
            log_width = (
                self.MAX_LOG_WIDTH
            )  # Use max width for logs to fit timestamp format
        ...
        ideal_commit_width = min(
            int(
                available_for_commits * 0.4
            ),  # 40% since short datetime is more compact
    ...
    def needs_recalculation(self, screen_width: int) -> bool:
        ...
        """
        return self.cached_widths is None or self.last_screen_width != screen_width
