...
class ScrollableMixin:
    ...
    def get_visible_range(
        self, viewport_height: int, border_size: int = 2
    ) -> Tuple[int, int, int]:
    ...
    def scroll_to_bottom(self, viewport_height: int, border_size: int = 2) -> None:
        ...
        if hasattr(self, "cursor_idx"):
            self.cursor_idx = min(
                len(self.items) - 1, self.scroll_offset + visible_count - 1
            )

    def calculate_items_that_fit(
        self, start_idx: int, item_heights: List[int], available_height: int
    ) -> int:
    ...
    def find_start_to_include_cursor(
        self, cursor_idx: int, item_heights: List[int], available_height: int
    ) -> int:
    ...
    def get_visible_range_variable(
        self, viewport_height: int, item_heights: List[int], border_size: int = 2
    ) -> Tuple[int, int, int]:
        ...
        if hasattr(self, "cursor_idx") and self.cursor_idx < n:
            cursor_height = (
                item_heights[self.cursor_idx]
                if self.cursor_idx < len(item_heights)
                else 0
            )
        ...
        if hasattr(self, "cursor_idx"):
            ...
            end_idx = self.calculate_items_that_fit(
                self.scroll_offset, item_heights, available_height
            )
